...
void FileHandler(std::string MapRepo, long int& TotalTileSurfaces);
...
bool SurfaceCreation(std::ifstream& sourceIMG, std::string& line, long int& TotalTilesOfSurface) {
	...
	if (sourceIMG.is_open()) {
		...
		while (!IsSameString(line.c_str(), "Collision")) { // go through txt file until you reach "collision"
			...
			new SurfaceProperty(tempName, tempSource, IMG_Load(TempPath), TempPath, TILE_WIDTH, TILE_HEIGHT, &gRenderer, &SurfacePropertyMap);
...
void GenerateTiles() {
	...
	for (auto p : SurfacePropertyMap) {
		...
		for (int j = 0; j < TotalCord; j++) {
			TotalTile[j] = new Tile(p.first, j, SurfaceWidth, SurfaceHeight, SurfacePropertyMap[p.first]->GetCollision(j), TILE_WIDTH, TILE_HEIGHT); // p.first is the "AA", j is the coordinate, ###, SourceX, SourceY
		}
	}
...
void FileHandler(std::string MapRepo, long int& TotalTilesOfSurface) {
	...
	if (sourceIMG.is_open()) { //Insert a check to see if the map was openable, if not throw error should be thrown, so I guess make this bool?
		...
		if (!init(gWindow, gScreenSurface, &gRenderer, SCREEN_WIDTH * TILE_WIDTH, SCREEN_HEIGHT * TILE_HEIGHT)) // gWindow, gScreenSurface, gRenderer, SCREEN_WIDTH * TILE_WIDTH, SCREEN_HEIGHT * TILE_HEIGHT))
		{
