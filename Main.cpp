	...
	while (!quit) {
		...
		while (SDL_PollEvent(&e) != 0) {	// this is checking to see if there is an event or not 
			...
			if (e.type == SDL_KEYDOWN && e.key.repeat == 0) // if the user holds down a key
			{
				...
				case SDLK_u: // if the user presses 'u' -- this will be utility for now, but can be used to display fps
					if (displayFPS) {
						displayFPS = false;
					}
					else {
		...
		// this just resets the time if it gets over 30
		if (TIME >= 30) {
			TIME = 0;
		}
...
int wmain(int argc, char* args[]) {
	...
	// handles the file that is used for creating the game	
