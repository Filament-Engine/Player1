...
public:
	...
	Text(const char* font_path, std::string message, int font_size, SDL_Color Color, int xPos, int yPos, bool Stay) { // this is the constructor for placing it on a level instead of camera position
		...
		{
			// here we create the rectangle for how the message will be displayed
			text_rect = new SDL_Rect;
			SDL_QueryTexture(textTexture, NULL, NULL, &text_rect->w, &text_rect->h); // gives us a width and height for the texture/rect
			text_rect->x = xPos;
			text_rect->y = yPos;
